...
namespace SegurosApp.API.Controllers
{
    ...
    public class AzureController : ControllerBase
    {
        ...
        public async Task<ActionResult> DebugHeaders()
        {
            try
            {
                var endpoint = _configuration["AzureDocumentIntelligence:Endpoint"];
                ...
                // Log exactamente qué estamos enviando
                _logger.LogInformation("🔍 DEBUG - Sending request:");
                _logger.LogInformation("🔍 Endpoint: {Endpoint}", endpoint);
                _logger.LogInformation("🔍 API Key first 10 chars: {ApiKeyPrefix}", apiKey?.Substring(0, Math.Min(10, apiKey?.Length ?? 0)));
                _logger.LogInformation("🔍 API Key length: {Length}", apiKey?.Length);
                ...
                // Log headers que estamos enviando
                _logger.LogInformation("🔍 Request URL: {Url}", testUrl);
                _logger.LogInformation("🔍 Headers being sent:");
                ...
                return Ok(new
                {
                    ...
                    requestHeaders = httpClient.DefaultRequestHeaders.ToDictionary(h => h.Key, h => string.Join(", ", h.Value)),
                    responseHeaders = response.Headers.ToDictionary(h => h.Key, h => string.Join(", ", h.Value))
                });
            }
            catch (Exception ex)
